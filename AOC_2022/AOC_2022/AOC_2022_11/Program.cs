...
foreach (var inputMonkey in inputMonkeys) {
    if (!string.IsNullOrEmpty(inputMonkey))
    {
        ...
        monkeys.Add(new Monkey
        {
            ...
            Items = new Queue<long>(monkey[1].Substring(18).Split(", ").Select(i => long.Parse(i))),
...
{
    var lcm = list.Select(m => m.DivideBy).Aggregate((a, b) => a * b);
    ...
    for (int round = 0; round < rounds;)
    {
        for (int i = 0; i < list.Count; i++)
        {
            while (list[i].Items.Count > 0)
            {
                ...
                var l = operation[0] == "old" ? oldWorryLevel : long.Parse(operation[0]);
                var r = operation[2] == "old" ? oldWorryLevel : long.Parse(operation[2]);
                var newWorryLevel = 0L;
                switch (operation[1])
                {
                    case "*":
                        newWorryLevel = l * r;
                ...
                var dividedWorryLevel = 0L;
                if (divide != 1)
                    dividedWorryLevel = newWorryLevel / divide;
                else
                    dividedWorryLevel = newWorryLevel % lcm;
